namespace SqliteApp
{
	...
	class AppDelegate : IUIApplicationDelegate
	{
		...
		public BOOL application(UIApplication application) didFinishLaunchingWithOptions(NSDictionary launchOptions)
		{
			...
			{
				var lSplitViewController = (UISplitViewController)this.window.rootViewController;
				var navigationController = lSplitViewController.viewControllers.lastObject();
				lSplitViewController.@delegate = (IUISplitViewControllerDelegate)navigationController.topViewController;
			}
			return true;
		}
