...
begin
  if UIDevice.currentDevice.userInterfaceIdiom = UIUserInterfaceIdiom.UIUserInterfaceIdiomPad then begin
    ...
    //contentSizeForViewInPopover := CGSizeMake(320.0, 600.0); //59091: Nougat: Linker error on CGSizeMake - we dont have inline functions yet
  end;
...
begin
  ...
  result := if assigned(fObjects) then fObjects.count else 0; //59096: Nougat: Compiler NRE when jusing ":" on a simple type
end;
...
begin
  ...
  //var lObject := fObjects[indexPath.row]; // 59210: Nougat: object subscripting support on older deployment targets
...
begin
  if segue.identifier.isEqualToString('showDetail') then begin
    ...
    //var lObject := fObjects[lIndexPath.row]; // 59210: Nougat: object subscripting support on older deployment targets
    var lObject := fObjects.objectAtIndex(lIndexPath.row);
    
   // segue.destinationViewController.setDetailItem(lObject); //59090: Nougat: cannot access members defined in project, on "id"
